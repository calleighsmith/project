...
$$
\begin{aligned}
\mu_i = \text{exp}(\text{ln}(t_i) &+ \beta_1 + \beta_2\times \text{beach length}_i +\beta_3\times I(\text{water body name}_i = \text{Chesapeake Bay}) \\
&+ \beta_4\times I(\text{water body name}_i = \text{Gulf of Mexico}) + \beta_5\times I(\text{water body name}_i = \text{Inland})\\
&+ \beta_6\times I(\text{water body name}_i = \text{Lake Erie}) + \beta_7\times I(\text{water body name}_i = \text{Lake Huron}) \\
&+ \beta_8\times I(\text{water body name}_i = \text{Lake Michigan}) + \beta_9\times I(\text{water body name}_i = \text{Lake Ontario})\\
&+ \beta_{10}\times I(\text{water body name}_i = \text{Lake Superior}) + \beta_{11}\times I(\text{water body name}_i = \text{Long Island Sound}) \\
&+ \beta_{12}\times I(\text{water body name}_i = \text{Pacific Ocean}) + \beta_{13}\times I(\text{swim status}_i = \text{No Advisory or Closure})\\
&+ \beta_{14}\times I(\text{beach access}_i = \text{Public}) + \beta_{15}\times I(\text{beach owner}_i = \text{Public}) + \beta_{16}\times \text{days in swim season}_i).
\end{aligned}
$$
...
Below one can find the results from the zero-inflated negative binomial model which predicts the numnber of days under beach action using features related to beach attributes.
...
labels <- c("Intercept", "Beach Length (mi)", "Water Body (Chesapeake Bay)", "Water Body (Gulf of Mexico)",
                  "Water Body (Inland)", "Water Body (Lake Erie)", "Water Body (Lake Huron)", 
                  "Water Body (Lake Michigan)", "Water Body (Lake Ontario)", "Water Body (Lake Superior)", "Water Body (Long Island Sound)", "Water Body (Pacific Ocean)", "Swim Status (No advisory or closure)",  "Beach Access (Public)",  "Beach Owner (Public)", "Swim Season (day)")
...
tab_model(m2, digits = 4, show.zeroinf = T,
          pred.labels = c(labels, labels),
...
```{r}
# check what the underlying count distribution is
Ord_plot(pollution$no_pollutants5)
```
# References
